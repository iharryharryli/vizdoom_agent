...
class RolloutStorage(object):
    def __init__(self, num_steps, num_processes, obs_shape, action_space, recurrent_hidden_state_size, state_size):
        ...
        self.esp = torch.zeros(num_steps, num_processes, state_size)
    ...
    def to(self, device):
        ...
        self.esp = self.esp.to(device)
        self.prev_action_one_hot = self.prev_action_one_hot.to(device)
        self.masks = self.masks.to(device)

    def insert(self, obs, recurrent_hidden_states, actions, esp, action_log_probs, value_preds, rewards, masks):
        self.obs[self.step + 1].copy_(obs)
        self.recurrent_hidden_states[self.step + 1].copy_(recurrent_hidden_states)
        self.actions[self.step].copy_(actions)
        self.esp[self.step].copy_(esp)
