...
for j in range(num_updates_init, num_updates):
    for step in range(parameters['num_steps']):
        # Sample actions
        with torch.no_grad():
            value, action, action_log_prob, recurrent_hidden_states, p_hidden_states = actor_critic.act(
                    rollouts.obs[step],
                    rollouts.recurrent_hidden_states[step],
                    rollouts.p_hidden_states[step],
                    rollouts.masks[step],
        ...
        masks = torch.FloatTensor([[0.0] if done_ else [1.0]
                                   for done_ in done])
        rollouts.insert(obs, recurrent_hidden_states, p_hidden_states, 
